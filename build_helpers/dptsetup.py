...
# Derived from the module of the same name in the DPT API source distribution
# directory <dptapi>/sample projects/DPT with Python/mingw_port.
# This module starts make jobs with the '-f <sibling path>/dptMakefile' option
# after the <...>/mingw_port directory becomes currect working directory.
...
if __name__ == '__main__':
    ...
    def GetCommandArgs():
        ...
        """
        optimize_options = {'-O0', '-O1', '-O2', '-O3', '-Os'}
        ptp = 'PATH_TO_PYTHON='
    ...
    def MakeTarget(target, defines, options, settings):
        ...
        if sys.platform != 'win32':
            # Maybe the mingw32 cross-compiler is available
            # Do not know if building in this way is that simple.
            command = 'gmake'
        else:
            command = 'make'
    ...
    if rc != 0:
        sys.stderr.write(' '.join(('Result code', str(rc))))
        sys.exit(rc)
